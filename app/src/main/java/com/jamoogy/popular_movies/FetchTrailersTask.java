...
public class FetchTrailersTask extends AsyncTask<Movie, Void, String[]> {
    ...
    protected String[] doInBackground(Movie... movies) {
        ...
        try {
            ...
            final String BASE_URL_WITH_SORT = TMDB_MOVIE_BASE_URL + movie.id + "/videos?";
        ...
        try {
            trailers = getMovieTrailersFromJson(movie, trailerDataJsonStr);
    ...
    private String[] getMovieTrailersFromJson(Movie movie, String trailerDataJsonStr)
        ...
        for(int i = 0; i < trailersArray.length(); i++) {
            ...
            JSONObject movieFromArray = trailersArray.getJSONObject(i);

            // Extract individual data for the movie and include it in the array
            String trailer_youtube_key = YOUTUBE_BASE_TRAILER_URL + movieFromArray.getString(TMDB_KEY);

            trailerURLs[i] = trailer_youtube_key;
        }
        return trailerURLs;
    }
}
